...
* 选择阿里云的源
按住Alt+F2，搜Software & Updates，选择软件源到阿里云的源，速度会快上很多。然后，就是标准的 =sudo apt-get update= 和 =sudo apt-get upgrade= 了。
...
我的电脑室x86的，我已经打包好了一个x86版本的deb包，可以去百度云盘上下载。
...
* 安装Atom
Atom没有提供x86版本的deb包，但是确实是支持x86的。64位版本的，在Atom的官网就能下载了。32位的电脑需要用PPA下载安装。

32位和64位的机器都可以使用PPA安装：
#+begin_src sh
...
* 安装Sublime
去官方网站 [[http://www.sublimetext.com/3][下载Sublim]]e 的deb安装包，右键用debi打开。点击 Install Package就可以了。

* 安装StartUML
StartUML是原作者近期开发的新的UML画图软件，保留了之前的强大功能的基础上，增加了更多功能和支持了多个操作系统。
...
* 安装pycharm
* 安装chrome
* 安装chrome的flash plugin
在Software Updates选择Canonical Parter的源。 =sudo apt-get update= 更新源头，然后 =sudo apt-get install adobe-flashplugin=

参考
- [[https://wiki.ubuntu.com/Chromium/Getting-Flash][Getting-Flash]]
